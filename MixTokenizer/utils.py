...
from MixTokenizer.core.segment_core import is_new_char_array, find_change_points, Trie
    ...
    class MixTokenizer(tokenizer_cls):
        ...
        def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
            ...
            if extra_config.get("mapping") and extra_config.get("used_ids"):
                ...
                instance.reverse_mapping = Trie()
                for point in instance.mapping:
                    instance.reverse_mapping.insert(point)
            else:
                raise ValueError(
        ...
        def _convert_ids_to_new_lang_id(self, token_ids: List[int] | np.ndarray) -> int:
            return self.reverse_mapping.lookup(token_ids)

        def _convert_ids_to_new_lang_id_batch(self, batch_token_ids: List[List[int]] | np.ndarray) -> List[int]:
            return self.reverse_mapping.batch_lookup(batch_token_ids)
